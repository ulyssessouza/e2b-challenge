...
func (c *countingResolver) GetUserByEmail(_ context.Context, _ string) (db.User, error) {
	c.calls++
	return c.user, c.err
}
...
func TestCachedUserResolverHitsCacheWithinTTL(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		user, err := cached.GetUserByEmail(context.Background(), "foo@bar.com")
		if err != nil {
...
func TestCachedUserResolverRefetchesAfterTTL(t *testing.T) {
	...
	_, _ = cached.GetUserByEmail(context.Background(), "foo@bar.com")
	_, _ = cached.GetUserByEmail(context.Background(), "foo@bar.com")
...
func TestCachedUserResolverPassesThroughErrors(t *testing.T) {
	...
	_, err := cached.GetUserByEmail(context.Background(), "foo@bar.com")
	if err == nil {
...
func TestCachedUserResolverRespectsCapacity(t *testing.T) {
	...
	_, _ = cached.GetUserByEmail(context.Background(), "a@x.com") // call 1, cached
	_, _ = cached.GetUserByEmail(context.Background(), "b@x.com") // call 2, evicts a
	_, _ = cached.GetUserByEmail(context.Background(), "a@x.com") // call 3, evicts b
...
func TestCachedUserResolverKeepsWorkingAfterFillingUp(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_, _ = cached.GetUserByEmail(context.Background(), string(rune('a'+i))+"@x.com")
	}
	// A repeat of the most recent user must still be served from cache.
	_, _ = cached.GetUserByEmail(context.Background(), "e@x.com")
